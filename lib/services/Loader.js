...
var Loader = function (_EventEmitter) {
  ...
  _createClass(Loader, [{
    ...
    value: function loadClients() {
      var clients = (0, _requireAll2.default)({
        dirname: _path2.default.join(process.cwd(), 'lib/clients'),
        filter: /(.+)\.js$/
      });
    ...
    value: function loadConfig() {
      ...
      var configPath = _path2.default.join(process.cwd(), 'config');
      _fs2.default.readdir(configPath, function (err, filenames) {
        if (err) _this4.logger.error('Error reading keys', err);

        var _loop = function _loop(filename) {
          _fs2.default.readFile(_path2.default.join(configPath, filename), 'utf-8', function (err, content) {
            if (err) _this4.logger.error('Error reading file ' + filename, err);
            if (!filename.startsWith('.') && filename.indexOf('example') === -1) {
              _this4.container.setParam(filename.substring(0, filename.indexOf('.')), content);
            }
          });
        };
