...
var Disco = function (_BaseClient) {
  ...
  _createClass(Disco, [{
    ...
    value: function queueSC(query, msg) {
      ...
      var cid = this.container.getParam('soundcloud');
      this.send(msg.channel, ':mag:  Queuing song...').then(function (msg) {
    ...
    value: function run() {
      ...
      client.Dispatcher.on('MESSAGE_CREATE', function (e) {
        ...
        _this6.responds(m, /^(youtube|yt) (q|find|search|query|\?) (.+)$/i, function (matches) {
          ...
          _superagent2.default.get('https://www.googleapis.com/youtube/v3/search?part=snippet&q=' + escape(matches[3]) + ('&key=' + _this6.container.getParam('google_api'))).end(function (err, res) {
            if (err) {
