...
var Anime = function (_BaseCommand) {
  ...
  _createClass(Anime, [{
    ...
    value: function genEntry(entry) {
      ...
      this.send(this.channel, ['\n:mag_right:  **' + entry.title + '**', entry.english ? '\n**Alternate Title**: ' + entry.english : '\n', '**Score**: ' + entry.score, '**Episodes**: ' + entry.episodes, '**Status**: ' + entry.status, '**Type**: ' + (entry.type || 'None'), '**Start Date**: ' + entry.start_date, '**End Date**: ' + entry.end_date, '**Synopsis**: ' + synopsis, 'http://myanimelist.net/anime/' + entry.id].join('\n'));
    }
  }, {
    key: 'handle',
