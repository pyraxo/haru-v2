...
var Anime = function (_BaseCommand) {
  ...
  _createClass(Anime, [{
    ...
    value: function genEntry(entry) {
      this.send(this.channel, ['\n:mag_right:  **' + entry.title + '**', entry.english ? '\n**English Title**: ' + entry.english : '\n', '**Score**: ' + entry.score, '**Episodes**: ' + entry.episodes, '**Status**: ' + entry.status, '**Type**: ' + (entry.type || 'None'), '**Start Date**: ' + entry.start_date, '**End Date**: ' + entry.end_date, '**Synopsis**: ' + (entry.synopsis[0] ? _he2.default.decode(entry.synopsis[0]).replace(/<br\s*[\/]?>/gi, '').replace(/\[\/*i\]/gi, '*').replace(/\[\/*b\]/gi, '**') : 'none'), 'http://myanimelist.net/anime/' + entry.id].join('\n'));
    }
  }, {
    key: 'handle',
      ...
      this.responds(/^anime (.+)$/i, function (matches) {
        ...
        _superagent2.default.get('http://myanimelist.net/api/anime/search.xml?q=' + query).auth(config.user, config.pass).accept('xml').buffer(true).parse(_superagentXml2jsparser2.default).end(function (err, res) {
          ...
          _this2.client.awaitResponse(_this2.message, reply.join('\n')).then(function (msg) {
            var num = parseInt(msg.content, 10) - 1;
            if (num >= entries.length) {
              num = entries.length - 1;
            }
            _this2.genEntry(entries[num]);
          });
        });
      });
    }
