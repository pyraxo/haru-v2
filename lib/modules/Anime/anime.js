...
var Anime = function (_BaseCommand) {
  ...
  _createClass(Anime, [{
    ...
    value: function genEntry(entry) {
      this.send(this.channel, ['\n:mag_right:  **' + entry.title + '**', entry.english ? '\n**English Title**: ' + entry.english : '\n', '**Score**: ' + entry.score, '**Episodes**: ' + entry.episodes, '**Status**: ' + entry.status, '**Type**: ' + (entry.type || 'None'), '**Start Date**: ' + entry.start_date, '**End Date**: ' + entry.end_date, '**Synopsis**: ' + (entry.synopsis[0] ? _he2.default.decode(entry.synopsis[0]).replace(/<br\s*[\/]?>/gi, '').replace(/\[\/*i\]\s*/gi, '*').replace(/\[\/*b\]\s*/gi, '**') : 'none'), 'http://myanimelist.net/anime/' + entry.id].join('\n'));
    }
  }, {
    key: 'handle',
