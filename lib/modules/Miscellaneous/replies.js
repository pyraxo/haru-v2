...
var _Datalist = require('../../util/Datalist');
...
var rc = new _Datalist2.default(_path2.default.join(process.cwd(), 'db/replies/reply_channels.json'));
  ...
  _createClass(Replies, [{
    ...
    value: function handle() {
      ...
      this.responds(/^replies list$/i, function () {
        ...
        _this4.reply(['List of expressions that will trigger a reply:', '`ayy, wew, ' + reply.join(', ') + '`'].join('\n'));
      });

      this.responds(/^replies$/i, function () {
        if (rc.has(_this4.channel.id)) {
          rc.del(_this4.channel.id);
          _this4.reply('Banned replies on this channel.');
        } else {
          rc.push(_this4.channel.id);
      ...
      if (rc.has(this.channel.id)) {
        ...
        this.hears(/^we(w+)$/i, function (matches) {
          _this4.send(_this4.channel, 'la' + Array(matches[1].length + 1).join('d'));
        });
      }
    }
  }], [{
