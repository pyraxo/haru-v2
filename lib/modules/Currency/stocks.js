      ...
      this.responds(/^stocks (.+)$/i, function (matches) {
        ...
        _superagent2.default.get('http://www.google.com/finance/info?q=' + symbol).end(function (err, res) {
          if (res.statusCode === 400) {
            _this2.send(_this2.channel, ':negative_squared_cross_mark:  Stock **$' + symbol.toUpperCase() + '** does not exist, **' + _this2.sender.name + '**!');
            return;
          } else if (err) {
            _this2.reply('**Error**: Fetching ' + symbol.toUpperCase() + ' met with ' + err);
            return;
          }
