...
var src = _fs2.default.readFileSync(_path2.default.join(process.cwd(), 'db/images/rip2.png'));
var fg = new _canvas.Image();
fg.src = src;
...
var RIP = function (_BaseCommand) {
  ...
  _createClass(RIP, [{
    ...
    value: function altImage(user) {
      ...
      if (_Cache2.default.has(user.id)) {
        this.genImage(_Cache2.default.get(user.id));
      } else {
        _nodeBase64Image2.default.base64encoder(user.avatarURL, { string: true }, function (err, image) {
          ...
          _Cache2.default.add(user.id, image);
    ...
    key: 'usage',
    get: function get() {
      return [['**rip** [text | user] - Here lies a dead soul']];
    }
  }]);
