...
var Cockfight = function (_BaseCommand) {
  ...
  _createClass(Cockfight, [{
    ...
    value: function listen() {
      ...
      _Cockfight.Den.once('BETTING_WON', function (winner, bets) {
        bets.forEach(function (bet) {
          ...
          _this2.send(_this2.channel, ':moneybag:  **' + _this2.client.users.get(bet.user) + '** has won **' + bet.amount + '** credits!');
        });
      });
      ...
      _Cockfight.Den.once('BETTING_LOST', function (loser, bets) {
        bets.forEach(function (bet) {
          ...
          _this2.send(_this2.channel, ':money_with_wings:  **' + _this2.client.users.get(bet.user) + '** has lost **' + bet.amount + '** credits!');
        });
      });
    }
  }, {
    ...
    key: 'usage',
    get: function get() {
      return ['```rb', '== COCKFIGHTS ==', '```', ['cockfight <user> - Engages in a cockfight with a user!', 'cockfight buy - Buys a :rooster: !']];
    }
  }]);
