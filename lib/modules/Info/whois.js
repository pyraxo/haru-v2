...
var Whois = function (_BaseCommand) {
  ...
  _createClass(Whois, [{
    ...
    value: function fetchUser(user) {
      ...
      return ['```xl', '================ [PROFILE] =================', 'Name: ' + user.username, 'ID: ' + user.id, 'Discriminator: ' + user.discriminator, 'Status: ' + user.status, 'Game: ' + (user.game ? user.game.name : 'none'), 'Bot: ' + user.bot, 'Roles: ' + (roles.length > 0 ? roles.join(', ') : 'none'), '============================================', '```'].join('\n');
    }
  }, {
    key: 'handle',
