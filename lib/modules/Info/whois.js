...
var Whois = function (_BaseCommand) {
  ...
  _createClass(Whois, [{
    ...
    value: function fetchUser(user) {
      ...
      var count = 0;
      this.client.servers.map(function (server) {
        if (server.members.get('id', user.id)) count++;
      });
      return ['```rb', '   User: ' + user.username + '#' + user.discriminator, '     ID: ' + user.id, '   Nick: ' + (this.server.detailsOf(user).nick || 'none'), ' Status: ' + user.status, 'Playing: ' + (user.game ? user.game.name : 'none'), user.bot ? '    Bot: ' + user.bot + '```' : '```', '`Shared servers`: ' + count, '`Created at: ' + (0, _moment2.default)(user.id / 4194304 + 1420070400000).format('ddd, Do MMMM YYYY, h:mm:ss a [GMT]Z') + '`', '`Joined at: ' + (0, _moment2.default)(this.server.detailsOf(user).joinedAt).format('ddd, Do MMMM YYYY, h:mm:ss a [GMT]Z') + '`', '`Roles`: ' + (roles.length > 0 ? roles.join(', ') : 'none')].join('\n');
    }
  }, {
    key: 'handle',
