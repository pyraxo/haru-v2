...
var BaseCommand = function () {
  ...
  _createClass(BaseCommand, [{
    ...
    key: 'await',
    value: function await(msg, prompt, check, errMsg) {
      var _this = this;

      return new Promise(function (res, rej) {
        _this.client.awaitResponse(msg, prompt).then(function (ans) {
          if (check(ans) === true) {
            return res(ans);
          } else {
            errMsg = errMsg || 'That is an invalid response. Please try again.';
            _this.await(ans, errMsg, check, errMsg).then(res).catch(rej);
          }
        }).catch(function (err) {
          return rej(err);
        });
    ...
    value: function send(dest, content) {
      ...
      if (delay) {
        return setTimeout(function () {
          _this2.send(dest, content, { delay: 0, deleteDelay: deleteDelay });
        }, delay);
      }
      ...
      return new Promise(function (res, rej) {
        _this2.client.sendMessage(dest, content, function (err, msg) {
          if (err) return rej(err);

          if (deleteDelay) {
            _this2.client.deleteMessage(msg, { wait: deleteDelay }, function (err) {
              if (err) return rej(err);
              if (!msgRem) res(msg);
            });
    ...
    value: function upload(attachment, name, channel) {
      ...
      return new Promise(function (res, rej) {
        _this3.client.sendFile(channel, attachment, name).then(function (msg) {
          return res(msg);
        }).catch(function (err) {
          return rej(err);
        });
    ...
    key: 'getMatches',
    value: function getMatches(content, regex, cb, print) {
    ...
    value: function hears(regex, callback) {
      var print = this.container.getParam('debug');
